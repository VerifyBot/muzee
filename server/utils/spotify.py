...
from asyncspotify import PlaylistTrack
...
class MyAuthenticator(sp.oauth.flows.Authenticator):
  def __init__(self, client: 'MySpotifyClient', client_id, client_secret, access_token):
...
class MyHTTP(sp.http.HTTP):
  async def refresh_token(self):
    ...
    async with aiohttp.ClientSession() as cs:
      async with cs.post("https://accounts.spotify.com/api/token", data=data) as resp:
        js = await resp.json()
...
class MySpotifyClient(sp.Client):
  ...
  def __init__(self,
    ...
    self.auth = MyAuthenticator(self, client_id=self.client_id, client_secret=self.client_secret,
  ...
  async def delete_playlist_tracks(self, playlist_id: str, tracks_ids: list[str]):
    ...
    while tracks_ids:
      await self.delete(f'playlists/{playlist_id}/tracks', tracks=[{'uri': uri} for uri in tracks_ids])
      tracks_ids = tracks_ids[limit:]
...
def sp_endpoint(keep_client: bool = False):
  ...
  def wrapper(func):
    async def inner(self: 'Spotify', *args, user: User = None, access_token: str = None, refresh_token: str = None,
                    **kwargs):
      assert user or (access_token and refresh_token), \
...
class Spotify:
  ...
  def __init__(self, client_id: str, client_secret: str, scope: str, server_url: str, db: Database):
    self.client_id = client_id
    self.client_secret = client_secret
    self.scope = scope
  ...
  async def get_access_token_from_code(self, code: str):
    ...
    async with aiohttp.ClientSession() as cs:
      async with cs.post("https://accounts.spotify.com/api/token", data=data, headers=headers) as resp:
        js = await resp.json()
  ...
  async def refresh_access_token(self, refresh_token: str):
    ...
    async with aiohttp.ClientSession() as session:
      async with session.post(self.API_ENDPOINT + "/oauth2/token", data=data, auth=auth) as resp:
        js = await resp.json()
