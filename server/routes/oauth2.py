...
from ..utils.spotify import OAuthError
...
async def route_connect(request: Request):
  ...
  return redirect("https://accounts.spotify.com/authorize?" + urlencode(dict(
    response_type="code",
...
async def route_callback(request: Request, ctx: Context):
  ...
  if set(js["scope"].split(" ")) != set(request.app.ctx.config.get("spotify", "scope").split(" ")):
  ...
  # get user info
  try:
    me = await ctx.spotify.fetch_user_info(access_token=access_token, refresh_token=refresh_token)
